...
export class ParticleManager {
  constructor({ maxParticles = 1000, maxTrailLength = 256 } = {}) {
  ...
  reset(id = null) {
    ...
    for (let index = 0; index < this.activeCount; index += 1) {
      this.particles[this.activeSlots[index]].reset();
    }
  ...
  clear() {
    for (let index = 0; index < this.activeCount; index += 1) {
      this.particles[this.activeSlots[index]].deactivate();
    }
    this.idToSlot.clear();
    this.activeCount = 0;
  ...
  update(delta) {
    ...
    for (let index = 0; index < this.activeCount; index += 1) {
      this.particles[this.activeSlots[index]].update(delta);
    }
    this.currentRevision += 1;
  }
  ...
  spawnBatch(definitions) {
    ...
    let createdCount = 0;
    try {
      for (; createdCount < definitions.length; createdCount += 1) {
        created[createdCount] = this.create(definitions[createdCount]);
      }
    } catch (error) {
  ...
  selected() {
    return this.selectedId === null ? null : this.findById(this.selectedId);
  }
